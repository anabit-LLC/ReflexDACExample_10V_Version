...
void setup()
{
    ...
    //Compiler directive if statement, runs single output voltage settings. Range is +vref to -vref
        ...
        uint16_t dacCode = dacBipolarVoltageToCode(DAC_14_BIT, REF_VOLT, outputThisVoltage); //convert voltage between 10V to -10V to 14 bit DAC code
...
void loop()
{
    ...
    //Compiler directive if statement, cycles DAC ouput from max to min. appears as squarewave
        ...
        quickChangeTest(DAC_14_BIT); //function loops between DAC max range points (+10V to -10V) as fast as possible
    #endif
}
...
void quickChangeTest(uint16_t bits) {
    ...
    while(1) {
        digitalWrite(PIN_CS, LOW);            // Set CS pin LOW
        if(val) {
            SPI.transfer16(bits);             //DAC max upper range code (+10V)
            val = false;
        }
        else {
            SPI.transfer16(0);                //DAC max lower range code (-10V)
            val = true;
        }
        digitalWrite(PIN_CS, HIGH);           // Set CS pin High 
    }
